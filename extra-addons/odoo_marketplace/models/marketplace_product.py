...
class ProductTemplate(models.Model):
    ...
    def toggle_website_published(self):
        ...
        for record in self:
            if record.status != 'approved' and not record.website_published:
...
class ProductProduct(models.Model):
    ...
    def get_seller_warehouse_ids(self):
        sellers = self.mapped('marketplace_seller_id')
        override_sellers = sellers.filtered(lambda s: s.set_seller_wise_settings)
        warehouse_objs = override_sellers.mapped('warehouse_id')
        warehouse_ids = warehouse_objs.ids
        if sellers - override_sellers:
            globel_warehouse_id = request.env['ir.default'].sudo().get('res.config.settings', 'mp_warehouse_id')
            if globel_warehouse_id:
                warehouse_ids.append(globel_warehouse_id)
        return warehouse_ids

    # def _get_domain_locations(self):
    #     warehouse = self.env.context.get('warehouse', False)
    #     if warehouse:
    #         warehouse = self.get_seller_warehouse_ids()
    #     return super(ProductProduct, self.with_context(warehouse=warehouse))._get_domain_locations()
